 ...
 * @licence MIT 2023 Eric Rykwalder, see https://github.com/erykwalder/lezer-markdown-obsidian/blob/6223674c535addf0fa60c8a04e3ebf5fd8aa3d7e/LICENSE
 */
...
export const InsertDelim: DelimiterType = {
  resolve: "Insert",
  mark: "InsertMarker",
};
...
export const YAMLFrontMatter: (options?: {
  allBlocks?: boolean;
}) => MarkdownConfig = ({ allBlocks } = {}) => ({
  ...
  parseBlock: [
    {
      ...
      parse(cx: BlockContextWithInput, line: Line) {
        ...
        if (
          line.text !== "---" ||
          cx.input.read(start + 3, start + 3 + 2) === "\n\n"
        )
          return false;

        const lastLineEnd = cx.input.read(
          cx.prevLineEnd() - 1,
          cx.prevLineEnd() + 1,
        );
  ...
  props: [
    ...
    foldNodeProp.add({
      YAMLFrontMatter(tree) {
        ...
        return {
          from: first.to,
          to: last.name == "YAMLMarker" ? last.to : tree.to,
        };
      },
    }),
  ],
